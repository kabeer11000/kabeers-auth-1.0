...
private $uniqueId = '';
private $url = '';
  function init ($r, $id, $m, $bg = 'fff', $force = 'true') {
    ...
    $this->$actual_link = $r;
        if($force === 'true'){
            $this->$url = 'http://auth.kabeersnetwork.rf.gd/?redirect='.$this->$actual_link.'&clientId='.encrypt($id,$x).'&action='.$m.'&key='.$x.'&bg='.$bg.'&f=true';
        }else{
