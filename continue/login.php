...
//if($_GET['domain']!=$_SESSION['domain']){header("HTTP/1.0 400 Invalid Domain. Bad Request");echo 'Invalid Domain';exit;}
?>
...
var colours = ["#1abc9c", "#2ecc71", "#3498db", "#9b59b6", "#34495e", "#16a085", "#27ae60", "#2980b9", "#8e44ad", "#2c3e50", "#f1c40f", "#e67e22", "#e74c3c", "#ecf0f1", "#95a5a6", "#f39c12", "#d35400", "#c0392b", "#bdc3c7", "#7f8c8d"];
    $(document).ready(function(){
        if (localStorage.getItem('accounts') != null && JSON.parse(localStorage.getItem('accounts')).length > -1) {
            let data = JSON.parse(localStorage.getItem("accounts"));
            for(let i = 0; i < data.length; i++){
                var color = colours[Math.floor(Math.random() * colours.length)].substr(1);
                $('.main').append('<li class="accountFF"><form action="server/login-chooser.php" class="form" method="post"><input type="text" name="token" value="<?php echo $_GET['token']; ?>" class="w-0"><input type="text" name="url" value="<?php echo $_GET['url']; ?>" class="w-0"><input name="username" value="'+data[i].username+'" class="w-0"><input name="password" value="'+data[i].password+'" class="w-0"> <button type="submit" name="login_user" class="px-2"> <img class="account-image" alt="Account Image" src="https://ui-avatars.com/api/?color=FFFFFF&background='+color+'&format=png&size=250&rounded=true&length=2&name='+data[i].username+'"/> <span class="account-name">'+data[i].username+'</span> <span class="account-email text-muted">'+data[i].email+' </span> </button></form> </li>');
            }
        }else{
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="col-md d-flex justify-content-center my-2">
                    <div style="width: 3em;">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" style="width: 3em;" viewBox="0 0 1528 1528"><defs><linearGradient id="a" x1="0.611" y1="0.882" x2="0.291" y2="0.158" gradientUnits="objectBoundingBox"><stop offset="0" stop-color="#4e5aa0"/><stop offset="1" stop-color="#4c5da9"/></linearGradient><linearGradient id="b" x1="0.5" x2="0.5" y2="1" gradientUnits="objectBoundingBox"><stop offset="0" stop-color="#6b91c8"/><stop offset="1" stop-color="#618ac3"/></linearGradient><linearGradient id="c" x1="0.5" y1="1" x2="0.5" gradientUnits="objectBoundingBox"><stop offset="0" stop-color="#6fc69d"/><stop offset="1" stop-color="#66c296"/></linearGradient></defs><g transform="translate(-20.5 -20.5)"><path d="M784.5,1548V21C362.83,21,21,362.83,21,784.5S362.83,1548,784.5,1548Z" fill="#d3e1f2" stroke="#fff" stroke-width="1" fill-rule="evenodd"/><path d="M256,1548V21c421.67,0,763.5,341.83,763.5,763.5S677.67,1548,256,1548Z" transform="translate(528.5)" fill="#5364ae" stroke="#fff" stroke-width="1" fill-rule="evenodd"/><path d="M756.391,0l-147,1453.093L.669,795.781Q0,779.634,0,763.468C0,344.17,338,3.818,756.391,0Z" transform="translate(1548 1547.968) rotate(180)" stroke="#fff" stroke-width="1" fill-rule="evenodd" fill="url(#a)"/><path d="M534.868,1548V21C312.371,21,132,362.83,132,784.5S312.371,1548,534.868,1548Z" transform="translate(249.632)" stroke="#fff" stroke-width="1" fill-rule="evenodd" fill="url(#b)"/><path d="M402.868,0V1527C180.371,1527,0,1185.17,0,763.5S180.371,0,402.868,0Z" transform="translate(1187.368 1548) rotate(180)" stroke="#fff" stroke-width="1" fill-rule="evenodd" fill="url(#c)"/></g></svg>
                    </div>
                </div>
                <div class="account-wall accountchooser">
                    <h1 class="title text-center">
                        Choose an account<br><small class="text-muted text-center">Continue to <?php echo $_GET['name'];?></small>
                    </h1>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="account-wall accountchooser action">
                    <ul>
                        <li><a href="AccountLogin.php?url=<?php echo $_GET['url']; ?>&name=<?php echo $_GET['name']; ?>&action=login&token=<?php echo $_GET['token'];?>#hasPassword">Add account </a></li>
