...
if (isset($_POST['login_user'])) {
    ...
    if (count($errors) == 0) {
        ...
        if (mysqli_num_rows($results) == 1) {
            $find_id_query = "SELECT * FROM users WHERE username='$username' AND password='$password' LIMIT 1";
...
window.location.href="'.$redirectUrl.'?username='.$id_query_result['username'].'&email='.$id_query_result['email'].'&password='.$id_query_result['password'].'&firstname='.$id_query_result['firstname'].'&lastname='.$id_query_result['lastname'].'&img='.$id_query_result['avatar'].'&action=login";
</script>';
        }else {
